...
- (IBAction)yolo:(UIButton*)sender
{
    ...
    UIImage *trans_img = [UIImage imageNamed:@"jbing"];
    
    
    [UIView animateWithDuration:0.2 animations:^{
        dope.alpha = 0.0f;
    } completion:^(BOOL finished) {
        dope.imageView.animationImages = [NSArray arrayWithObjects:trans_img,nil];
        [dope.imageView startAnimating];
        [UIView animateWithDuration:0.2 animations:^{
            dope.alpha = 1.0f;
        }];
    }];
  ...
  // [dope setImage:[UIImage imageNamed:@"jbing"] forState:UIControlStateNormal];
    /*
