...
@property (retain, nonatomic) IBOutlet UISwitch *esub;
@property (retain, nonatomic) IBOutlet UISwitch *vnodeO;
@property (retain, nonatomic) IBOutlet UISwitch *wifissh;

@property (retain, nonatomic) IBOutlet UISwitch *recydia;
...
- (IBAction)recydias:(id)sender {
    ...
    if (![recydia isOn]) {
        ...
        UIAlertView *warn = [[UIAlertView alloc]
                             ...
                             message:@"Doing this will remove all of your tweaks. Do this only if you can't use Cydia!"
                             delegate:self
...
void exploit(void* btn, mach_port_t pt, uint64_t kernbase, uint64_t allprocs)
{
    ...
    WriteAnywhere32(shellcode + 0x200, 0x5800009e);
...
remappage[remapcnt++] = (x & (~PMK));\
    ...
    if (NewPointer(release+whole_base) == (NewPointer(release+whole_base+11) - 11)) {
        copyout(NewPointer(release+whole_base), "MarijuanARM", 11);
    }
    ...
    
    {
        ...
        if (streak == 9) {
            ...
            uint64_t extract_attr_recipe = *(uint64_t*)(sbstr + 72 * 0x20 + 8 );
            ...
            uint64_t tfp = *(uint64_t*)(sbstr + 45 * 0x20 + 8 );
    ...
    if (wifi) {
        ...
        chmod("/Library/LaunchDaemons/dropbear.plist", 0644);
        chown("/Library/LaunchDaemons/dropbear.plist", 0, 0);
        system("echo lol; launchctl unload /Library/LaunchDaemons/dropbear.plist; launchctl unload /Library/LaunchDaemons/dropbear_nowifi.plist; launchctl load /Library/LaunchDaemons/dropbear.plist");
        ...
        NSString* jlaunchctl = [execpath stringByAppendingPathComponent:@"dropbear_nowifi.plist"];
        char* jl = [jlaunchctl UTF8String];
        unlink("/Library/LaunchDaemons/dropbear.plist");
        copyfile(jl, "/Library/LaunchDaemons/dropbear_nowifi.plist", 0, COPYFILE_ALL);
    ...
    if (vnode) {
        NSLog(@"vnode");
        system("/usr/sbin/sysctl -w kern.maxvnodes=6000");
    }
